...
//            String from = prop.get("mail.smtp.username") + "";
     ...
     * @throws MessagingException 发邮件异常
     */
    public String sendMailC(JavaMailSender javaMailSender, String ftlName,Map<String,Object> args, String subject, String emailAdress, Configuration freemarkerConfig) throws MessagingException {
        MimeMessage mMessage = javaMailSender.createMimeMessage();// 创建邮件对象
        MimeMessageHelper mMessageHelper;
        //获得邮件文本
        String text = getTextC(freemarkerConfig,ftlName,args);
     ...
     * @return 邮件内容文本
     */
    private String getTextC(Configuration freemarkerConfig,String ftlName,Map<String,Object> args){
        String text = "";
        try {
            //获得email.ftl模板
            Template template = freemarkerConfig.getTemplate(ftlName);
            //传入参数构建模板文本
            text = FreeMarkerTemplateUtils.processTemplateIntoString(template,args);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (TemplateException e) {
            e.printStackTrace();
